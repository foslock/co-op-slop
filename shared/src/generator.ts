...
function placePlatform(s: GenState, theme: ThemeDef, zi: number, gap: number, dy: number, kind: PathStep['kind'], archOverride?: string, alignToPath = false): Vec3 {
  const archId = archOverride ?? s.rng.pick(theme.pathProps.filter((p) => ARCHETYPES[p].pathable));
  ...
  s.steps.push({ from: { ...s.cur }, to: { ...top }, kind, gap, dy, zone: zi });
  s.nodes.push({ ...top });
  s.cur = top;
...
function placeJumpStep(s: GenState, theme: ThemeDef, zi: number) {
  const r = s.rng.float();
  ...
  placePlatform(s, theme, zi, gap, dy, 'jump');
}

/** Widest prop in a theme, so gadget landings are never a pinpoint hop. */
function wideArch(s: GenState, theme: ThemeDef): string {
  const wide = theme.pathProps.filter((p) => ARCHETYPES[p].pathable && ARCHETYPES[p].topRadius >= 1.1);
  return s.rng.pick(wide.length > 0 ? wide : theme.pathProps);
}

function placeBridge(s: GenState, theme: ThemeDef, zone: number) {
  ...
  const far = placePlatform(s, theme, zone, span, 0, 'gadget', wideArch(s, theme));
  const farR = s.curR;
...
function placeLadder(s: GenState, theme: ThemeDef, zi: number) {
  const id = s.gadgetId++;
  const from = { ...s.cur };
  const fromR = s.curR;
  const rise = s.rng.float(5.5, 7.5);
  const top = placePlatform(s, theme, zi, -s.curR * 0.4, rise, 'gadget'); // nearly directly above
...
function placeTraverse(s: GenState, theme: ThemeDef, zi: number) {
  const id = s.gadgetId++;
  const near = { ...s.cur };
  const nearR = s.curR;
  const span = s.rng.float(8, 11.5) / gravityScaleAtZone(zi);
  const far = placePlatform(s, theme, zi, span, 0, 'gadget', wideArch(s, theme));
  const farR = s.curR;
  const dir = Math.atan2(far.z - near.z, far.x - near.x);
  ...
  s.gadgets.push({
    kind: 'traverse',
    id,
    a: v(near.x + cos * nearR * 0.8, near.y + TRAVERSE.anchorHeight, near.z + sin * nearR * 0.8),
    b: v(far.x - cos * farR * 0.8, far.y + TRAVERSE.anchorHeight, far.z - sin * farR * 0.8),
    deckY: near.y,
  });
}

function placeRope(s: GenState, theme: ThemeDef, zi: number) {
  const id = s.gadgetId++;
  const from = { ...s.cur };
  const rise = s.rng.float(6.5, 9);
  const top = placePlatform(s, theme, zi, -s.curR * 0.2, rise, 'gadget');
  const topR = s.curR;
...
function placeItemBranch(s: GenState, theme: ThemeDef, zi: number, type: ItemType) {
  ...
  const archId = s.rng.pick(theme.pathProps.filter((p) => ARCHETYPES[p].pathable));
  const arch = ARCHETYPES[archId];
...
export function generateLevelDebug(seed: string): { level: LevelData; steps: PathStep[] } {
  ...
  for (let zi = 0; zi < order.length; zi++) {
    ...
    if (zi > 0) {
      ...
      const top = placePlatform(s, theme, zi, rng.float(1.2, 2.0), rng.float(0.8, 1.2), 'jump', 'checkpoint_pad', true);
    ...
    for (let n = 0; n < nodes; n++) {
      ...
      if (sinceGadget >= nextGadgetAt && zi + n > 2) {
        ...
        if (kind < 0.3) placeBridge(s, theme, zi);
        else if (kind < 0.55) placeLadder(s, theme, zi);
        else if (kind < 0.75) placeRope(s, theme, zi);
        else placeTraverse(s, theme, zi);
        sinceGadget = 0;
        nextGadgetAt = rng.int(5, 8);
      } else {
        placeJumpStep(s, theme, zi);
      }
      if (itemNodes.has(n)) {
        placeItemBranch(s, theme, zi, ITEM_CYCLE[itemCycle++ % ITEM_CYCLE.length]);
      }
    }
  ...
  const summit = placePlatform(s, order[order.length - 1], order.length - 1, rng.float(1.0, 1.8), rng.float(1.0, 1.4), 'jump', 'summit_pad');
